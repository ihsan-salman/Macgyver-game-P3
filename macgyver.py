...
class Macgyver:
    ...
    def display(self):
        ...
        self.macgyver = pygame.image.load(
            constants.IMAGE_MACGYVER).convert_alpha()
        self.labyrinth.screen.blit(self.macgyver,
                                   (self.pos_x * constants.SPRITE_SIZE,
                                    self.pos_y * constants.SPRITE_SIZE))

    def display_font(self):
        # Deleting last hero position's picture
        self.font = pygame.image.load(
            constants.IMAGE_FONT_SPRITE).convert_alpha()
    ...
    def move_right(self):
        ...
        if (self.pos_x < (constants.NB_SPRITE - 1)) and (
             self.labyrinth.structure[self.pos_y][self.pos_x + 1] != 'm'):
    ...
    def move_left(self):
        ...
        if (self.pos_x > 0) and (self.labyrinth.structure[
                                 self.pos_y][self.pos_x - 1] != 'm'):
            self.display_font()
            self.pos_x -= 1
            self.display()
            self.collect(self.items)
    ...
    def move_up(self):
        # Same method as move_right
        if (self.pos_y > 0) and (self.labyrinth.structure[
                                 self.pos_y - 1][self.pos_x] != 'm'):
            self.display_font()
            self.pos_y -= 1
            self.display()
            self.collect(self.items)
    ...
    def move_down(self):
        # Same method as move_right
        if (self.pos_y < (
            constants.NB_SPRITE - 1)) and (self.labyrinth.structure[
                                           self.pos_y + 1][
                                           self.pos_x] != 'm'):
            self.display_font()
            self.pos_y += 1
    ...
    def collect(self, items):
        ...
        self.syringe = pygame.image.load(
            constants.IMAGE_SYRINGE).convert_alpha()

        # Collect condition
        if (self. pos_y, self.pos_x) == items.\
                needle_position and 'needle' not in self.bag:
            # Adding item to the bag
            self.bag.append('needle')
            # display the item in the down black field
            # to show the contenant of the bag
            self.labyrinth.screen.blit(self.needle, (100, 660))
            print('You have a needle', len(self.bag))

        elif (self.pos_y, self.pos_x) == items.\
                syringe_position and 'syringe' not in self.bag:
            self.bag.append('syringe')
            self.labyrinth.screen.blit(self.syringe, (300, 660))
            print('You have a syringe', len(self.bag))

        elif (self.pos_y,
              self.pos_x) == items.ether_position and 'ether' not in self.bag:
