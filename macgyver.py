...
class Macgyver:
	...
	def display(self):
		...
		self.labyrinth.screen.blit(self.macgyver, (self.pos_x * \
			constants.SPRITE_SIZE, self.pos_y * constants.SPRITE_SIZE))
	

	def display_font(self):
		# Deleting last hero position's picture method
		self.labyrinth.screen.blit(self.font, (self.pos_x * \
			constants.SPRITE_SIZE, self.pos_y * constants.SPRITE_SIZE))
	...
	def collect(self, items):
		...
		if [self. pos_y, self.pos_x] == self.items.needle_pos:
			...
			# Change the position of the item to be sure that can be collect just one time
