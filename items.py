...
class Items:
    ...
    def item_position(self):
        ...
        while len(self.list_position) != 3:
            ...
            if self.labyrinth.structure[self.y][
               self.x] != 'm' and (self.y, self.x) not in self.list_position:
    ...
    def display(self):
        ...
        self.labyrinth.screen.blit(
            self.needle, (self.needle_position[1] * constants.SPRITE_SIZE,
                          self.needle_position[0] * constants.SPRITE_SIZE))
        self.syringe_position = self.list_position[1]
        self.labyrinth.screen.blit(
            self.syringe, (self.syringe_position[1] * constants.SPRITE_SIZE,
                           self.syringe_position[0] * constants.SPRITE_SIZE))

        self.ether_position = self.list_position[2]
        self.labyrinth.screen.blit(
            self.ether, (self.ether_position[1] * constants.SPRITE_SIZE,
                         self.ether_position[0] * constants.SPRITE_SIZE))